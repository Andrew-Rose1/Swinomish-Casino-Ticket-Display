...
Changelog:
-- Version 2.0.0
    --- Use SQL query to grab open ticket numbers. Then make API calls on those tickets.

-- Version 1.4.7
    --- Created a local DB, openTicketsDB.txt which is used to keep track of open tickets.
    --- Created rescanDB, which scans local DB instead of range of open tickets
-- Version 1.4.6
    --- Mounted AutoVM's authToken.txt. All API Pi will point there from now on.
-- Version 1.4.5
    --- Move firstTicketScan to a serperate function
    --- Outline overdue tickets in red/orange
 ...
 -- Version 1.4.4
    --- Will NOT make API calls between 10:58pm and 11:02pm, because of Shift Report Email that scans att 11pm (NO LONGER NEEDED)
 -- Version 1.4.3
...
class Window(Frame):
    def __init__(self, master = None):
        ...
        # Define auhorization key location
        self.authTokenPath = "/mnt/AutoVM_Scripts/authToken.txt"
        ...
        # Time constraints to withhold from scanning
    ...
    # Authorize API token
    def authorize(self):
        tokenFile = open(self.authTokenPath, "r")
    ...
    # Generate new API authorization token
    def getNewToken(self):
        ...
        tokenFile = open(self.authTokenPath, "w")
    ...
    def rescanTicketDB(self, myHeader, db):
        self.openTickets.clear()
        openTicketsArr = self.queryOpened(db)
        self.vendorCounter = 0
        self.enduserCounter = 0
        self.pausedCounter = 0
        self.partsCounter = 0
        for ticket in openTicketsArr:
            url = 'http://10.0.1.4/TrackIt/WebApi/tickets/' + str(ticket[0])
            response = requests.get(url, headers=myHeader)
            if response:
                ticketJSON = response.json()
                # Ensure ticket is from group "Help Desk"
                try:
                    if ((ticketJSON["Ticket"]["34"]["Value"] == "HELP DESK") or (ticketJSON["Ticket"]["34"]["Value"] == "SYSTEM ADMINISTRATION")):
                        if (ticketJSON["Ticket"]["96"]["Value"] == "Open") and not (ticketJSON["Ticket"]["22"]["Value"].startswith("WSUS - Updates Server Group")):
                            url2 = 'http://10.0.1.4/TrackIt/WebApi/tickets/' + str(ticket[0]) + '/Notes/0/0'
        ...
        self.openTickets.sort(key = lambda x: int(x.id))
        self.openTickets.sort(key = lambda x: int(x.priority))
        


    # Countdown timer until next scan
    ...
    def init_window(self):
        ...
        try:
            ...
            url = 'http://10.0.1.4/TrackIt/WebApi/tickets/344'
        ...
        db = pyodbc.connect("Driver=FreeTDS;Server=10.0.1.4;Database=TRACKIT2020DB;UID=API;PWD=********;PORT=1433;TDS_Version=8.0")
        self.rescanTicketDB(authorizedHeader, db)
